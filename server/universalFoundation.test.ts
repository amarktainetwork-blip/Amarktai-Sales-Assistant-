...
describe("connection-secret encryption", () => {
  ...
  beforeEach(() => {
    process.env.CONNECTION_SECRETS_MASTER_KEY = Buffer.alloc(32, 7).toString(
      "base64"
    );
  });
  afterEach(() => {
    if (previous === undefined)
      delete process.env.CONNECTION_SECRETS_MASTER_KEY;
  ...
  it("round-trips credentials without retaining plaintext in the envelope", () => {
    ...
    expect(
      decryptConnectionSecret<{ accessToken: string; refreshToken: string }>(
        encrypted
      )
    ).toEqual({ accessToken: "access-secret", refreshToken: "refresh-secret" });
  });
  ...
  it("round-trips an encrypted browser replay package comfortably above 64 KB", () => {
    ...
    const encrypted = encryptConnectionSecret({
      browserSession: {
        ...
        sessionStorageByOrigin: {
          "https://crm.example.test": { auth: largeSessionValue },
        },
    ...
    expect(
      decryptConnectionSecret<{
        browserSession: {
          sessionStorageByOrigin: Record<string, { auth: string }>;
        };
      }>(encrypted).browserSession.sessionStorageByOrigin[
        "https://crm.example.test"
      ].auth
    ).toHaveLength(160_000);
    ...
    expect(
      validateSavedBrowserScript({
        steps: [
          {
            action: "goto",
            value: "https://crm.example.test/contact/{{contactId}}",
          },
          { action: "expect_visible", selector: "[data-record]" },
        ],
      }).steps
    ).toHaveLength(2);
  });
