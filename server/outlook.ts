...
export function getOutlookReadiness() {
  ...
  const clientConfigured = Boolean(process.env.OUTLOOK_CLIENT_ID?.trim() && process.env.OUTLOOK_CLIENT_SECRET?.trim());
...
async function fetchWithTimeout(url: string, init: RequestInit, timeoutMs = OUTLOOK_TIMEOUT_MS) {
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), timeoutMs);
  try { return await fetch(url, { ...init, signal: controller.signal }); }
  catch (error) {
    if (error instanceof Error && error.name === "AbortError") throw new Error("Microsoft Graph request timed out.");
...
async function graphRequest(url: string, init: RequestInit) {
  for (let attempt = 0; attempt < 2; attempt += 1) {
    ...
    if ((response.status === 429 || response.status >= 500) && attempt === 0) {
      ...
      await new Promise(resolve => setTimeout(resolve, Math.min(2, Number.isFinite(retryAfter) ? retryAfter : 1) * 1000));
      continue;
    }
...
export async function createOutlookApplicationToken() {
  ...
  const response = await graphRequest(`https://login.microsoftonline.com/${encodeURIComponent(process.env.OUTLOOK_TENANT_ID!.trim())}/oauth2/v2.0/token`, {
    method: "POST",
    headers: { "Content-Type": "application/x-www-form-urlencoded" },
    body: new URLSearchParams({ client_id: process.env.OUTLOOK_CLIENT_ID!.trim(), client_secret: process.env.OUTLOOK_CLIENT_SECRET!, grant_type: "client_credentials", scope: "https://graph.microsoft.com/.default" }),
  });
...
async function graphPost(path: string, body: Record<string, unknown>) {
  ...
  const response = await graphRequest(graphSenderEndpoint(path), { method: "POST", headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" }, body: JSON.stringify(body) });
...
export async function createOutlookCalendarEvent(input: CalendarEventInput) {
  ...
  const attendees = Array.from(new Set(input.attendees.map(email => email.trim().toLowerCase()).filter(email => /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email))));
  ...
  return { created: true as const, provider: "outlook" as const, eventId: typeof result === "object" && result && "id" in result && typeof result.id === "string" ? result.id : undefined };
}
