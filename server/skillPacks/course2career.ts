...
export const COURSE2CAREER_SKILL_PACK: readonly OrganisationSkillSeed[] = [
  ...
  },
  {
    ...
    title: "Invalid contact details — Stage 1",
    definition: definition(
      "When a same-day call proves the phone number is invalid or belongs to the wrong person, prepare only the confirmed Stage 1 unsuccessful-attempt record and Invalid Phone Number email. Keep every downstream closure rule explicitly incomplete until Course2Career authorises it.",
      "A same-day call genuinely proves the current phone number is invalid or belongs to the wrong person.",
      [
        ...readContext,
        {
        ...
        },
        {
          ...
          action: "prepare_note",
          label: "Prepare a factual unsuccessful-attempt note using only the verified call outcome",
        },
      ],
      {
        stopConditions: [
          "A corrected number is received; return to the normal First Call skill",
          "The phone has not actually been verified as invalid or wrong-person",
          "The live customer cannot be identified exactly",
        ],
        decisionRules: [
          "Only the confirmed Stage 1 behaviour is valid production logic",
          "Prepare Consultant Contact Emails / Invalid Phone Number after the same-day invalid/wrong-person call is verified",
          "Do not invent or perform SMS routing, task closure, opportunity closure, Lost status, Closed Lost field behaviour or any later-day workflow",
          "Downstream behaviour remains NEEDS_RULE_COMPLETION until authorised Course2Career evidence proves it",
        ],
        requiredMappings: [
        ...
        parameters: {
          ...
          confirmedScope: "same-day invalid/wrong-person call; factual note; Invalid Phone Number email preparation only",
        },
        assertions: [
          "No SMS or WhatsApp is prepared by this incomplete rule",
          "No task is completed or rescheduled by this incomplete rule",
          "No opportunity or contact field is changed by this incomplete rule",
          "No closure, Lost status or Closed Lost behaviour is inferred",
          "Genie remains read-only until each required write is separately commissioned",
        ],
      }
    ),
  },
