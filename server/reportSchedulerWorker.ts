...
export function cronMatchesUtc(expression: string, now: Date) {
  const fields = expression.trim().split(/\s+/); if (fields.length !== 6) return false;
...
async function deliver(report: typeof dailyReports.$inferSelect) {
  ...
  const [team, settings] = await Promise.all([getTeamIntelligence({ userId: report.userId, organisationId: membership.organisationId }), loadManagementReportSettings(membership.organisationId)]);
  const breaches = team.people.filter(person =>
    person.targetStatus === "at_risk" || person.targetStatus === "needs_attention"
...
async function runDueReports() {
  ...
  try {
    ...
    for (const report of reports) {
      ...
      const key = deliveryKey(report.id, now); if (report.lastDeliveryKey === key) continue;
  ...
  } catch (error) { console.error(JSON.stringify({ event: "report_scheduler_error", detail: error instanceof Error ? error.message : String(error) })); }
