...
function cleanRole(value: unknown): ManagedRole {
  if (value === "manager" || value === "salesperson" || value === "auditor")
    return value;
...
async function sendInviteEmail(input: {
  ...
  if (!getSmtpReadiness().ready)
    throw new Error("SMTP is required before team invitations can be sent.");
  const appUrl = process.env.APP_PUBLIC_URL?.replace(/\/$/, "");
  if (!appUrl)
    throw new Error(
      "APP_PUBLIC_URL is required before team invitations can be sent."
    );
...
function sendError(res: Response, error: unknown) {
  ...
  return res
    .status(400)
    .json({ error: detail.slice(0, 300) || "Team operation failed." });
}
...
export function registerTeamAdminRoutes(app: Express) {
    ...
    try {
      ...
      const policy =
        (
          await db
            .select()
            .from(organisationCompliancePolicies)
            .where(
              eq(
                organisationCompliancePolicies.organisationId,
  ...
  app.put("/api/team-admin/compliance-policy", async (req, res) => {
    try {
      ...
      const transcriptRetentionDays = boundedDays(
        req.body?.transcriptRetentionDays,
        90
      );
      const auditRetentionDays = boundedDays(req.body?.auditRetentionDays, 365);
      const operationalRetentionDays = boundedDays(
        req.body?.operationalRetentionDays,
        365
      );
      const outboundConsentRequired =
        req.body?.outboundConsentRequired === undefined
          ? true
          : Boolean(req.body.outboundConsentRequired);
      const deletionApprovalRequired =
        req.body?.deletionApprovalRequired === undefined
          ? true
          : Boolean(req.body.deletionApprovalRequired);
      const policyText =
        typeof req.body?.policyText === "string"
          ? req.body.policyText.trim().slice(0, 12_000) || null
          : null;
      const db = await getDb();
      if (!db) throw new Error("Database connection is unavailable.");
      await db
        .insert(organisationCompliancePolicies)
        .values({
          organisationId: membership.organisationId,
          transcriptRetentionDays,
          auditRetentionDays,
          operationalRetentionDays,
          outboundConsentRequired,
      ...
      await recordAudit({
        userId: actor.id,
        eventType: "compliance_policy_saved",
        entityType: "organisation_compliance_policy",
        entityId: String(membership.organisationId),
        summary: "Organisation compliance and retention policy was updated.",
        metadata: {
          organisationId: membership.organisationId,
          transcriptRetentionDays,
          auditRetentionDays,
    ...
    try {
      ...
      const [identityConnections, entitlement] = await Promise.all([
        db
          ...
          .where(
            eq(
              enterpriseIdentityConnections.organisationId,
              membership.organisationId
            )
          ),
        db
          .select()
          .from(organisationEntitlements)
          .where(
            eq(
              organisationEntitlements.organisationId,
              membership.organisationId
            )
          )
          .limit(1),
  ...
  app.put("/api/team-admin/enterprise-identity", async (req, res) => {
    try {
      const { user: actor, membership } = await requireManager(req);
      const protocol =
        req.body?.protocol === "saml" || req.body?.protocol === "scim"
          ? req.body.protocol
          : null;
      const displayName =
        typeof req.body?.displayName === "string"
          ? req.body.displayName.trim().slice(0, 180)
          : "";
      const configuration =
        req.body?.configuration &&
        typeof req.body.configuration === "object" &&
        !Array.isArray(req.body.configuration)
          ? (req.body.configuration as Record<string, unknown>)
          : {};
      if (!protocol || !displayName)
        throw new Error("Protocol and display name are required.");
      const db = await getDb();
      if (!db) throw new Error("Database connection is unavailable.");
      await db
      ...
      await recordAudit({
        userId: actor.id,
        eventType: "enterprise_identity_configured",
        entityType: "enterprise_identity_connection",
        entityId: protocol,
        summary: `${protocol.toUpperCase()} configuration saved as draft pending verification.`,
        metadata: { organisationId: membership.organisationId, protocol },
      });
      return res.json({ ok: true });
    } catch (error) {
      return sendError(res, error);
  ...
  app.get("/api/team-admin/data-subject-requests", async (req, res) => {
    try {
      ...
      const requests = await db
        ...
        .where(
          eq(dataSubjectRequests.organisationId, membership.organisationId)
        );
  ...
  app.post("/api/team-admin/data-subject-requests", async (req, res) => {
    try {
      const { user: actor, membership } = await requireManager(req);
      const requestType =
        req.body?.requestType === "export" ||
        req.body?.requestType === "deletion"
      ...
      const subjectType = [
        ...
        "user",
        "operational_record",
      ].includes(req.body?.subjectType)
        ? req.body.subjectType
        : null;
      const subjectReference =
        typeof req.body?.subjectReference === "string"
          ? req.body.subjectReference.trim().slice(0, 220)
          : "";
      const reason =
        typeof req.body?.reason === "string"
          ? req.body.reason.trim().slice(0, 6_000) || null
          : null;
      if (!requestType || !subjectType || !subjectReference)
        throw new Error(
          "Request type, subject type, and subject reference are required."
        );
      const db = await getDb();
      if (!db) throw new Error("Database connection is unavailable.");
      ...
      await recordAudit({
        userId: actor.id,
        eventType: "data_subject_request_created",
        entityType: "data_subject_request",
        entityId: String(result[0].insertId),
        summary: `${requestType} request queued for manager review.`,
        metadata: {
          organisationId: membership.organisationId,
          subjectType,
          subjectReference,
      ...
      return res
        .status(201)
        .json({ id: Number(result[0].insertId), status: "review_required" });
    } catch (error) {
      return sendError(res, error);
    }
  });
  ...
  app.put(
    "/api/team-admin/data-subject-requests/:id/review",
    async (req, res) => {
      try {
        const { user: actor, membership } = await requireManager(req);
        ...
        if (!Number.isInteger(id) || id <= 0 || !decision)
          throw new Error("A valid request and review decision are required.");
        const db = await getDb();
        if (!db) throw new Error("Database connection is unavailable.");
        const request = (
          await db
            ...
            .where(
              and(
                ...
                eq(
                  dataSubjectRequests.organisationId,
                  membership.organisationId
                )
              )
            )
        ...
        if (!request)
          throw new Error(
            "Data-subject request was not found in the active organisation."
          );
        if (request.status !== "review_required")
          throw new Error("Only requests awaiting review can be decided.");
        await db
        ...
        await recordAudit({
          userId: actor.id,
          eventType: "data_subject_request_reviewed",
          entityType: "data_subject_request",
          entityId: String(id),
          summary: `Data-subject ${request.requestType} request ${decision}.`,
          metadata: {
            organisationId: membership.organisationId,
            decision,
  ...
  app.post("/api/team-admin/playbook-versions", async (req, res) => {
    try {
      const { user: actor, membership } = await requireManager(req);
      const playbookKey =
        typeof req.body?.playbookKey === "string"
          ? req.body.playbookKey
              .trim()
              .toLowerCase()
              .replace(/[^a-z0-9_-]+/g, "-")
              .slice(0, 140)
      ...
      const instructions =
        typeof req.body?.instructions === "string"
          ? req.body.instructions.trim().slice(0, 20_000)
          : "";
      const inputSchema =
        req.body?.inputSchema &&
        typeof req.body.inputSchema === "object" &&
        !Array.isArray(req.body.inputSchema)
          ? (req.body.inputSchema as Record<string, unknown>)
          : {};
      if (!playbookKey || !title || !instructions)
        throw new Error("Playbook key, title, and instructions are required.");
      const db = await getDb();
      if (!db) throw new Error("Database connection is unavailable.");
      ...
      await recordAudit({
        userId: actor.id,
        eventType: "playbook_version_created",
        entityType: "playbook_version",
        entityId: String(result[0].insertId),
        summary: `Draft playbook version ${version} created.`,
        metadata: {
          organisationId: membership.organisationId,
          playbookKey,
          version,
      ...
      return res
        .status(201)
        .json({ id: Number(result[0].insertId), version, status: "draft" });
    } catch (error) {
      return sendError(res, error);
    }
  });
  ...
  app.put("/api/team-admin/playbook-versions/:id/publish", async (req, res) => {
    try {
      ...
      if (!Number.isInteger(id) || id <= 0)
        throw new Error("A valid playbook version is required.");
      const db = await getDb();
      if (!db) throw new Error("Database connection is unavailable.");
      const version = (
        await db
      ...
      await db.transaction(async tx => {
        await tx
          ...
          .where(
            and(
              eq(playbookVersions.organisationId, membership.organisationId),
              eq(playbookVersions.playbookKey, version.playbookKey),
      ...
      await recordAudit({
        ...
        entityType: "playbook_version",
        entityId: String(id),
        summary: `Playbook version ${version.version} published.`,
        metadata: {
          organisationId: membership.organisationId,
    ...
    try {
      ...
      const [jobs, receipts] = await Promise.all([
        db
          ...
          .where(
            eq(connectorSyncJobs.organisationId, membership.organisationId)
          ),
        db
          .select()
          .from(connectorWebhookReceipts)
          .where(
            eq(
              connectorWebhookReceipts.organisationId,
              membership.organisationId
            )
          ),
      ]);
    ...
    try {
      ...
      const resourceType =
        typeof req.body?.resourceType === "string"
          ? req.body.resourceType.trim().slice(0, 80)
          : "";
      const scheduleExpression =
        typeof req.body?.scheduleExpression === "string"
          ? req.body.scheduleExpression.trim().slice(0, 120)
          : "";
      const capabilityKey =
        typeof req.body?.capabilityKey === "string"
          ? req.body.capabilityKey.trim().slice(0, 120)
          : "";
      if (
        ...
        throw new Error(
          "Connected system, resource, schedule, and verified capability are required."
        );
      const db = await getDb();
      if (!db) throw new Error("Database connection is unavailable.");
      const system = (
      ...
      const verified =
        system.status === "ready" &&
        system.verifiedCapabilities.includes(capabilityKey);
      await db
        ...
        .onDuplicateKeyUpdate({
          set: {
            ...
            lastError: verified
              ? null
              : "Connector is not ready or capability is not verified.",
          },
        });
      await recordAudit({
        userId: actor.id,
        eventType: "connector_sync_job_saved",
        entityType: "connector_sync_job",
        entityId: `${connectedSystemId}:${resourceType}`,
        summary: `Connector sync job saved as ${verified ? "ready" : "draft"}.`,
        metadata: {
          organisationId: membership.organisationId,
          connectedSystemId,
          resourceType,
      ...
      return res
        .status(201)
        .json({ ok: true, status: verified ? "ready" : "draft" });
    } catch (error) {
      return sendError(res, error);
    }
  });
    ...
    try {
      ...
      const [messages, drafts] = await Promise.all([
        db
          ...
          .where(eq(inboundMessages.organisationId, membership.organisationId)),
        db
          .select()
          .from(inboundReplyDrafts)
          .where(
            eq(inboundReplyDrafts.organisationId, membership.organisationId)
          ),
      ]);
  ...
  app.post("/api/team-admin/inbound-review/:id/drafts", async (req, res) => {
    try {
      ...
      const id = Number(req.params.id);
      const draftBody =
        typeof req.body?.draftBody === "string"
          ? req.body.draftBody.trim().slice(0, 20_000)
          : "";
      const rationale =
        typeof req.body?.rationale === "string"
          ? req.body.rationale.trim().slice(0, 6_000)
          : "";
      if (!Number.isInteger(id) || id <= 0 || !draftBody || !rationale)
        throw new Error(
          "A valid message, draft text, and rationale are required."
        );
      const db = await getDb();
      if (!db) throw new Error("Database connection is unavailable.");
      const message = (
        await db
          ...
          .where(
            and(
              eq(inboundMessages.id, id),
              eq(inboundMessages.organisationId, membership.organisationId)
            )
          )
          .limit(1)
      )[0];
      if (!message)
        throw new Error(
          "Inbound message was not found in the active organisation."
        );
      ...
      if (classification.category === "unsubscribe")
        throw new Error(
          "Automated sales replies are suppressed for inbound unsubscribe requests."
        );
      const result = await db
        .insert(inboundReplyDrafts)
        .values({
          organisationId: membership.organisationId,
          inboundMessageId: id,
          draftBody,
      ...
      await recordAudit({
        userId: actor.id,
        eventType: "inbound_reply_draft_created",
        entityType: "inbound_reply_draft",
        entityId: String(result[0].insertId),
        summary:
          "Review-only inbound reply draft created; no send action was performed.",
        metadata: {
          organisationId: membership.organisationId,
          inboundMessageId: id,
          category: classification.category,
    ...
    try {
      ...
      return res.json({
        rubrics: await db
          .select()
          .from(qaRubrics)
          .where(eq(qaRubrics.organisationId, membership.organisationId)),
      });
    } catch (error) {
      return sendError(res, error);
    }
    ...
    try {
      ...
      const name =
        typeof req.body?.name === "string"
          ? req.body.name.trim().slice(0, 180)
          : "";
              ...
              return {
                key:
                  typeof row.key === "string"
                    ? row.key.trim().slice(0, 80)
                    : "",
                label:
                  typeof row.label === "string"
                    ? row.label.trim().slice(0, 180)
                    : "",
                weight: Number(row.weight),
              };
            })
            .filter(
              (item: { key: string; label: string; weight: number }) =>
                item.key &&
                item.label &&
      ...
      if (!name || !criteria.length)
        throw new Error(
          "A rubric name and at least one weighted criterion are required."
        );
      const db = await getDb();
      if (!db) throw new Error("Database connection is unavailable.");
      const result = await db
      ...
      await recordAudit({
        userId: actor.id,
        eventType: "qa_rubric_created",
        entityType: "qa_rubric",
        entityId: String(result[0].insertId),
        summary: "Quality-assurance rubric created.",
        metadata: {
          organisationId: membership.organisationId,
  ...
  app.get("/api/team-admin/crm-owner-mappings", async (req, res) => {
    try {
      ...
      const connectedSystemId =
        req.query.connectedSystemId === undefined
          ? undefined
          : Number(req.query.connectedSystemId);
      if (
        connectedSystemId !== undefined &&
      ...
      if (connectedSystemId)
        predicates.push(
          eq(externalUserMappings.connectedSystemId, connectedSystemId)
        );
      const mappings = await db
        .select({
          ...
          isActive: externalUserMappings.isActive,
          userId: externalUserMappings.userId,
          memberName: users.name,
          memberEmail: users.email,
        })
        .from(externalUserMappings)
        .leftJoin(users, eq(users.id, externalUserMappings.userId))
    ...
    try {
      ...
      const externalUserId =
        typeof req.body?.externalUserId === "string"
          ? req.body.externalUserId.trim().slice(0, 180)
          : "";
      const displayName =
        typeof req.body?.displayName === "string"
          ? req.body.displayName.trim().slice(0, 220)
          : "";
      const email =
      ...
      if (!externalUserId || !displayName)
        throw new Error("External owner ID and display name are required.");
      if (userId !== null && (!Number.isInteger(userId) || userId <= 0))
        throw new Error("A valid Amarktai team member is required.");
      const db = await getDb();
      if (!db) throw new Error("Database connection is unavailable.");
      const system = (
        await db
          .select({ id: connectedSystems.id })
          .from(connectedSystems)
          .where(
            and(
              eq(connectedSystems.id, connectedSystemId),
      ...
      if (userId !== null) {
        ...
        if (!member)
          throw new Error(
            "Mapped Amarktai member must be active in the organisation."
          );
      }
      await db
        ...
        .onDuplicateKeyUpdate({
          set: { userId, displayName, email, isActive },
        });
      await recordAudit({
        userId: actor.id,
        eventType: "crm_owner_mapping_saved",
        entityType: "external_user_mapping",
        entityId: `${connectedSystemId}:${externalUserId}`,
        summary: `CRM owner '${displayName}' mapping was saved.`,
        metadata: {
          organisationId: membership.organisationId,
          connectedSystemId,
          externalUserId,
  ...
  app.get("/api/team-admin/crm-pipeline-stage-mappings", async (req, res) => {
    try {
      ...
      if (!db) throw new Error("Database connection is unavailable.");
      const predicates = [
        eq(crmPipelineStageMappings.organisationId, membership.organisationId),
      ];
      if (connectedSystemId)
        predicates.push(
          eq(crmPipelineStageMappings.connectedSystemId, connectedSystemId)
        );
      const mappings = await db
        .select()
  ...
  app.put("/api/team-admin/crm-pipeline-stage-mappings", async (req, res) => {
    try {
      ...
      const externalPipelineId =
        typeof req.body?.externalPipelineId === "string"
          ? req.body.externalPipelineId.trim().slice(0, 180)
          : "";
      const externalStageId =
        typeof req.body?.externalStageId === "string"
          ? req.body.externalStageId.trim().slice(0, 180)
          : "";
      const pipelineLabel =
        typeof req.body?.pipelineLabel === "string"
          ? req.body.pipelineLabel.trim().slice(0, 220)
          : "";
      const stageLabel =
        typeof req.body?.stageLabel === "string"
          ? req.body.stageLabel.trim().slice(0, 220)
          : "";
      const category =
        typeof req.body?.category === "string" ? req.body.category : "other";
      const isActive =
        req.body?.isActive === undefined ? true : Boolean(req.body.isActive);
      if (!Number.isInteger(connectedSystemId) || connectedSystemId <= 0)
        throw new Error("A valid connected system is required.");
      if (
        ...
        throw new Error(
          "Pipeline ID, stage ID, pipeline name, and stage name are required."
        );
      if (
        !(
          ["open", "qualified", "proposal", "won", "lost", "other"] as const
        ).includes(
          category as
            | "open"
            | "qualified"
  ...
  app.post("/api/team-admin/invite", async (req, res) => {
    try {
      if (!isLocalAuthMode())
        throw new Error(
          "Self-hosted team invitations are available in local authentication mode."
        );
      const { user: actor, membership } = await requireManager(req);
      const email =
        typeof req.body?.email === "string"
      ...
      if (membership.role === "manager" && role === "manager")
        throw new Error(
          "Only an organisation owner can invite another manager."
        );
      const db = await getDb();
      if (!db) throw new Error("Database connection is unavailable.");
      ...
      if (!invited) {
        const inserted = await db
          .insert(users)
          .values({
            openId: `local-invite:${randomUUID()}`,
            name,
            email,
            loginMethod: "local",
            passwordHash: null,
      ...
      if (!invited.passwordHash) {
        ...
        await sendEmail({
          to: email,
          subject: `You've been added to ${membership.organisationName} on Amarktai`,
          text: `You have been added to ${membership.organisationName} on Amarktai. Sign in with your existing Amarktai credentials.`,
          html: `<main style="font-family:Arial,sans-serif;color:#102238"><h1>You've been added to ${membership.organisationName}</h1><p>Sign in with your existing Amarktai credentials.</p></main>`,
        });
        emailState = "existing_account_notified";
      }
      await recordAudit({
        userId: actor.id,
        eventType: "organisation_member_invited",
        entityType: "organisation_member",
        entityId: String(invited.id),
        summary: `${email} was added to the organisation as ${role}.`,
        metadata: {
          organisationId: membership.organisationId,
          role,
          emailState,
  ...
  app.patch("/api/team-admin/members/:memberId", async (req, res) => {
    try {
      ...
      if (!Number.isInteger(memberId) || memberId <= 0)
        throw new Error("A valid organisation member is required.");
      const db = await getDb();
      if (!db) throw new Error("Database connection is unavailable.");
      const current = (
        await db
          ...
          .where(
            and(
              ...
              eq(organisationMembers.organisationId, membership.organisationId)
            )
          )
          .limit(1)
      )[0];
      ...
      if (current.role === "owner")
        throw new Error(
          "The organisation owner cannot be changed from this screen."
        );
      if (current.userId === actor.id && req.body?.isActive === false)
        throw new Error(
          "You cannot deactivate your own management membership."
        );
      ...
      if (req.body?.role !== undefined) {
        ...
        if (membership.role === "manager" && role === "manager")
          throw new Error(
            "Only an organisation owner can assign the manager role."
          );
        patch.role = role;
      }
      ...
      if (!Object.keys(patch).length)
        throw new Error("No member change was supplied.");
      await db
      ...
      await recordAudit({
        userId: actor.id,
        eventType: "organisation_member_updated",
        entityType: "organisation_member",
        entityId: String(memberId),
        summary: "Organisation member access was updated.",
        metadata: {
          organisationId: membership.organisationId,
          role: patch.role,
  ...
  app.post("/api/team-admin/accept-invite", async (req, res) => {
    try {
      if (!isLocalAuthMode())
        throw new Error(
          "This invitation cannot be accepted in the current authentication mode."
        );
      ...
      if (
        payload.type !== "amarktai_team_invite" ||
        !payload.sub ||
      ...
      if (!user || user.email?.toLowerCase() !== payload.email.toLowerCase())
        throw new Error("This invitation no longer matches an account.");
      if (user.passwordHash)
        throw new Error(
          "This invitation has already been used. Sign in with your existing password."
        );
      const membership = (
        await db
          .select()
          ...
          .where(
            and(
              eq(organisationMembers.organisationId, payload.organisationId),
              eq(organisationMembers.userId, userId),
              eq(organisationMembers.isActive, true)
            )
          )
      ...
      await db
        .update(users)
        .set({ passwordHash, loginMethod: "local" })
      ...
      return res
        .status(400)
        .json({
          error: /JWT|expired|signature/i.test(detail)
            ? "This invitation is invalid or expired. Ask management to send a new one."
            : detail.slice(0, 300),
        });
    }
  });
