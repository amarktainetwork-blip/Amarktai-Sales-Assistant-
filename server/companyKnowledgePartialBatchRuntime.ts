...
export class PartialBatchWholeSiteModel implements WholeSiteLearningModel {
  ...
  private async analyseBatch(batch: CompanyInlineCorpusBatch) {
    ...
    try {
      const prompt = `This is bounded corpus batch ${batch.index + 1}/${batch.total}.\n\nReturn a PARTIAL CompanyKnowledgePack. Use only top-level keys for facts actually present in this batch. OMIT the company key if company identity is not directly evidenced here. OMIT absent top-level categories instead of emitting placeholder objects. Empty arrays are allowed, but never return objects with blank required strings. Every included fact and offering must cite only PAGE_XXXX IDs visible in this batch. Preserve contradictory first-party facts; do not silently choose one. Career programmes are first-class sales entities: preserve every distinct career programme directly evidenced in this batch as type career_programme, even when it contains multiple courses. Do not demote a career programme to an individual course. Use the first-party page title or H1 as the offering name when it clearly identifies the programme/course, and never emit placeholder or blank offering names.\n\nNested objects must follow the exact structures shown in this reference schema:\n${companyKnowledgeRepairTargetPrompt("analysis")}\n\nFor this partial response you MAY OMIT any top-level key not evidenced in this batch, including company. All offering text-list fields (including duration, support, certifications and outcomes) should be arrays of plain strings. Do not rename keys, add metadata, wrap the JSON, or output commentary.`;
