...
/** Compact, reusable context from a verified CRM read. It avoids repeated browser research for the same lead. */
export const crmContextSnapshots = mysqlTable("crmContextSnapshots", {
  id: int("id").autoincrement().primaryKey(),
  userId: int("userId").notNull().references(() => users.id, { onDelete: "cascade" }),
  leadLabel: varchar("leadLabel", { length: 180 }).notNull(),
  source: mysqlEnum("source", ["genie_browser", "manual"]).notNull(),
  ...
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull(),
}, table => [uniqueIndex("crmContextSnapshots_user_lead_uq").on(table.userId, table.leadLabel), index("crmContextSnapshots_user_expiry_idx").on(table.userId, table.expiresAt)]);

/** A human-approved communications draft. It is never a record of an email that was sent. */
export const communicationDrafts = mysqlTable("communicationDrafts", {
  id: int("id").autoincrement().primaryKey(),
  userId: int("userId").notNull().references(() => users.id, { onDelete: "cascade" }),
  leadLabel: varchar("leadLabel", { length: 180 }),
  recipientEmail: varchar("recipientEmail", { length: 320 }).notNull(),
  subject: varchar("subject", { length: 300 }).notNull(),
  body: text("body").notNull(),
  purpose: varchar("purpose", { length: 160 }).notNull(),
  dedupeKey: varchar("dedupeKey", { length: 64 }).notNull().default(""),
  qualityChecks: json("qualityChecks").$type<Array<{ key: string; passed: boolean; detail: string }>>().notNull(),
  state: mysqlEnum("state", ["draft", "review_required", "approved", "rejected"]).default("review_required").notNull(),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  reviewedAt: timestamp("reviewedAt"),
}, table => [index("communicationDrafts_user_state_idx").on(table.userId, table.state), index("communicationDrafts_user_dedupe_idx").on(table.userId, table.dedupeKey)]);

/** Persisted checks generated by the Manager Assurance Agent from factual workspace records. */
export const managerFindings = mysqlTable("managerFindings", {
  id: int("id").autoincrement().primaryKey(),
  userId: int("userId").notNull().references(() => users.id, { onDelete: "cascade" }),
  findingKey: varchar("findingKey", { length: 255 }).notNull(),
  severity: mysqlEnum("severity", ["critical", "high", "normal", "info"]).notNull(),
  state: mysqlEnum("state", ["open", "acknowledged", "resolved"]).default("open").notNull(),
  title: varchar("title", { length: 220 }).notNull(),
  ...
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull(),
}, table => [uniqueIndex("managerFindings_user_key_uq").on(table.userId, table.findingKey), index("managerFindings_user_state_idx").on(table.userId, table.state)]);

/** Cost and usage ledger for model-backed agent work. Values are provider-reported when available. */
export const agentUsageEvents = mysqlTable("agentUsageEvents", {
  id: int("id").autoincrement().primaryKey(),
  userId: int("userId").notNull().references(() => users.id, { onDelete: "cascade" }),
  agentKey: varchar("agentKey", { length: 80 }).notNull(),
  ...
  inputChars: int("inputChars").notNull(),
  outputChars: int("outputChars").notNull(),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
}, table => [index("agentUsageEvents_user_created_idx").on(table.userId, table.createdAt), index("agentUsageEvents_user_agent_idx").on(table.userId, table.agentKey)]);

/** Short-lived approved-response cache keyed by a compacted, policy-versioned agent request. */
export const agentResponseCache = mysqlTable("agentResponseCache", {
  id: int("id").autoincrement().primaryKey(),
  userId: int("userId").notNull().references(() => users.id, { onDelete: "cascade" }),
  agentKey: varchar("agentKey", { length: 80 }).notNull(),
  ...
  expiresAt: timestamp("expiresAt").notNull(),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
}, table => [uniqueIndex("agentResponseCache_lookup_uq").on(table.userId, table.agentKey, table.requestHash, table.policyVersion), index("agentResponseCache_user_expiry_idx").on(table.userId, table.expiresAt)]);
...
export type CrmContextSnapshot = typeof crmContextSnapshots.$inferSelect;
export type CommunicationDraft = typeof communicationDrafts.$inferSelect;
export type ManagerFinding = typeof managerFindings.$inferSelect;
