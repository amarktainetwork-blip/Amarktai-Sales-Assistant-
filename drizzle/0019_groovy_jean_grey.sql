...
ALTER TABLE `browserLearnedOperations` ADD CONSTRAINT `browserLearnedOperations_connectedSystemId_connectedSystems_id_fk` FOREIGN KEY (`connectedSystemId`) REFERENCES `connectedSystems`(`id`) ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
...
ALTER TABLE `contactCommunicationSuppressions` ADD CONSTRAINT `contactCommunicationSuppressions_organisationId_organisations_id_fk` FOREIGN KEY (`organisationId`) REFERENCES `organisations`(`id`) ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
ALTER TABLE `contactCommunicationSuppressions` ADD CONSTRAINT `contactCommunicationSuppressions_connectedSystemId_connectedSystems_id_fk` FOREIGN KEY (`connectedSystemId`) REFERENCES `connectedSystems`(`id`) ON DELETE set null ON UPDATE no action;--> statement-breakpoint
ALTER TABLE `contactCommunicationSuppressions` ADD CONSTRAINT `contactCommunicationSuppressions_sourceMessageId_inboundMessages_id_fk` FOREIGN KEY (`sourceMessageId`) REFERENCES `inboundMessages`(`id`) ON DELETE set null ON UPDATE no action;--> statement-breakpoint
