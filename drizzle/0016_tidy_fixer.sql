...
ALTER TABLE `playbookExecutionHistory` ADD CONSTRAINT `playbookExecutionHistory_playbookVersionId_playbookVersions_id_fk` FOREIGN KEY (`playbookVersionId`) REFERENCES `playbookVersions`(`id`) ON DELETE restrict ON UPDATE no action;--> statement-breakpoint
ALTER TABLE `playbookExecutionHistory` ADD CONSTRAINT `playbookExecutionHistory_approvalTemplateId_approvalTemplates_id_fk` FOREIGN KEY (`approvalTemplateId`) REFERENCES `approvalTemplates`(`id`) ON DELETE set null ON UPDATE no action;--> statement-breakpoint
