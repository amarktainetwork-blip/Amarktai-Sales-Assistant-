...
Use Ubuntu 24.04, a non-root sudo user, SSH keys, Docker Engine and the Docker Compose plugin. Expose only SSH, HTTP and HTTPS.
...
## 2. Clone the canonical production repository
...
Record the deployed SHA in the client handover. Never deploy a `main` SHA whose CI release gates are not green.

## 3. Recommended one-command guided install

For the normal full Webdock deployment with self-hosted Chromium:

```bash
AMARKTAI_DEPLOY_PROFILE=full sh deploy/webdock/quick-install.sh
```

The guided installer automatically generates strong internal values for:

- MariaDB application password;
- MariaDB root password;
- JWT signing secret;
- application secret;
- the 32-byte connection-credential encryption key.

It asks only for the client-specific values required for the core application:

- public hostname;
- administrator name/email/password;
- GenX chat-completions URL, API key and model ID;
- SMTP host/port/security/username/password/from identity.

It derives `APP_PUBLIC_URL`, forces self-hosted local auth, writes `.env` with mode `0600`, runs preflight, builds the selected Compose profile, applies migrations, starts services, waits for readiness and runs the internal smoke test automatically.

For the smaller pilot profile with an external authorised Playwright-compatible CDP endpoint:

```bash
AMARKTAI_DEPLOY_PROFILE=pilot sh deploy/webdock/quick-install.sh
```

The pilot wizard additionally asks for the external CDP endpoint.

Do not commit `.env` or paste production secrets into chat, tickets or screenshots. Protect `CONNECTION_SECRETS_MASTER_KEY` separately; losing it prevents encrypted connection credentials from being decrypted.

## 4. Manual configuration/install path

Experienced operators may instead use:
...
The standard installer runs preflight, validates Compose, builds images, starts infrastructure, applies versioned migrations through `node dist/migrate.js`, starts the application/workers, waits for internal readiness and runs `deploy/webdock/smoke-test.sh` before returning success.

## 5. Core configuration and optional providers

SMTP is required for email second factor, recovery, invitations and reports. GenX is required for the Sales Assistant.
...
## 6. DNS/TLS and production verification

Point `DOMAIN` to the VPS. Caddy obtains and renews TLS automatically.
...
`PRODUCTION_VERIFIER=PASS` and `PLATFORM_READY=PASS` prove the deployment/runtime only. `CLIENT_ACCEPTANCE=PENDING` is expected before client CRM commissioning. Complete the single controlled customer sign-in inside the Secure CRM Browser, then run `deploy/webdock/verify-client-acceptance.sh`; do not hand over until it reports `CLIENT_ACCEPTANCE=PASS`.
...
4. Complete OAuth or, for Genie, save only the authorised URL and sign in directly inside the private Secure CRM Browser. Amarktai never asks for or stores the CRM password. Install-level `GENIE_*` credentials and raw profile JSON are not required for the normal path.
...
## 16. Client handover acceptance

Minimum handover proof:

- deployed `main` SHA recorded and CI-green;
