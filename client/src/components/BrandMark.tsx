...
export function BrandMark({ compact = false, inverse = false, large = false }: BrandMarkProps) {
  ...
  return <div className="inline-flex items-center gap-3" aria-label="Amarktai Network Sales Assistant">
    ...
    {!compact && <span className={`grid leading-none ${name}`}><span className={`font-display font-bold tracking-[-.065em] ${large ? "text-[27px]" : "text-[21px]"}`}>Amarktai <em className="not-italic text-[#75A7FF]">Network</em></span><small className={`mt-1 text-[8px] font-black uppercase tracking-[.2em] ${subtext}`}>Sales Assistant</small></span>}
  </div>;
}
