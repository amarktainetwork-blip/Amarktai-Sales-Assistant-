...
export default function GenieInteractiveAuthPrompt({
  ...
  const [challenge, setChallenge] =
    useState<InteractiveCommissioning | null>(null);
  ...
  useEffect(() => {
    ...
    const check = async () => {
      try {
        ...
        // Company onboarding reports ordinary commissioning failures. This
  ...
  const memberGate = <MemberOnboardingGate />;
  if (
    !enabled ||
    !genie?.id ||
    (!approved && !challenge?.interactiveAuthRequired)
  )
    return memberGate;
  ...
  async function verify() {
    ...
    try {
      ...
      if (/MANAGEMENT_ELEVATION_/.test(detail))
        ...
        setError(
          "That Genie verification request expired. Request a new code below."
        );
      else if (/REJECTED|INVALID/.test(detail))
        setError(
          "Genie did not accept that code. Check the newest code and try again."
        );
      else if (/GENIE_PERSISTENT_PROFILE_IN_USE/.test(detail))
        setError(
          "This deployment's trusted Genie browser is bound to a different CRM connection. Amarktai blocked the session from being shared."
        );
      else if (
        /GENIE_SESSION_REPLAY_FAILED|GENIE_PERSISTENT_PROFILE_UNAVAILABLE|GENIE_AUTHENTICATED_PAGE_UNAVAILABLE/.test(
          detail
        )
      )
        setError(
          "Genie accepted the sign-in, but the exact approved browser tab is no longer usable. Do not request repeated codes; the browser runtime needs attention."
        );
        ...
        setError(
          "Genie verification could not be completed. Retry Verify once before requesting another code."
        );
    } finally {
      setPending(false);
    }
  ...
  async function requestNewCode() {
    ...
    try {
      ...
      if (result.interactiveAuthRequired)
        ...
        setApproved(true);
        toast.success(
          "Genie is already authenticated. CRM setup is continuing."
        );
        window.setTimeout(() => setApproved(false), 6_000);
      }
    } catch (cause) {
      ...
      if (/MANAGEMENT_ELEVATION_/.test(detail))
        ...
        setError(
          "This deployment's trusted Genie browser is already bound to another CRM connection and cannot be shared."
        );
      else if (/GENIE_AUTHENTICATED_PAGE_UNAVAILABLE/.test(detail))
        setError(
          "The approved Genie tab is no longer available. Do not request another code until the browser runtime has been checked."
        );
      else
        setError("A new Genie verification code could not be requested yet.");
    } finally {
      setPending(false);
    }
  }
  ...
  async function checkReadiness() {
    ...
    try {
      ...
      setError("");
      const result = (await jsonRequest(
        `/api/connected-system-admin/${genie.id}/pre-otp`,
        { method: "POST", body: "{}" }
      )) as PreOtpReadiness;
      setReadiness(result);
      ...
      setError(
        cause instanceof Error
          ? cause.message
          : "Secure sign-in readiness could not be proved."
      );
    } finally {
      setPending(false);
    }
          ...
          <div className="mt-4 space-y-3">
            <div className="grid gap-2 sm:grid-cols-2">
              {(
                ...
                <div
                  key={key}
                  className="flex items-center justify-between rounded-lg bg-black/15 px-3 py-2 text-[11px]"
                >
                  <span>{label}</span>
                  <span
                    className={
                      readiness?.states[key]
                        ? "font-bold text-emerald-200"
                        : "text-[#7896C1]"
                    }
                  >
                    {readiness?.states[key] ? "Ready" : "Not checked"}
                  </span>
                </div>
              ))}
            </div>
            ...
            >
              {pending ? "Checking readiness…" : "Check secure sign-in readiness"}
            </Button>
          </div>
        )}
