...
import { AlertCircle, Mail, RefreshCw, Reply, UserRound } from "lucide-react";
...
function readableMessage(body: string) {
  ...
  doc
    .querySelectorAll("blockquote, style, script, head, img, .gmail_quote")
...
export default function Inbox() {
  ...
  const syncInbox = trpc.sales.syncInbox.useMutation({
    ...
    onError: () =>
      toast.warning(
        "Inbox refresh is busy. The last synchronized messages remain available."
      ),
  });
          ...
          <div className="amk-inbox-list overflow-hidden rounded-2xl border border-[#DCE4EE] bg-white shadow-sm">
            <div className="border-b border-[#E6EBF2] px-5 py-4">
              ...
              <p className="mt-1 text-xs text-[#7A899C]">
                Newest source messages appear first.
              </p>
            </div>
            {inbox.isLoading ? (
              <p className="p-6 text-sm text-[#66758A]">
                    ...
                    >
                      <div className="flex items-center gap-2">
                        <span className="truncate font-bold text-[#26354A]">
                          {message.contact?.name || message.senderReference}
                        </span>
                        {category === "sale_intent" ? (
                          <span className="shrink-0 rounded-full bg-emerald-100 px-2 py-0.5 text-[10px] font-black uppercase tracking-[.08em] text-emerald-800">
                            Possible sale
                          </span>
                        ) : message.needsAction ? (
                      ...
                      <p className="mt-1 truncate text-sm font-semibold text-[#526985]">
                        {message.subject ||
                          `${message.channel.toUpperCase()} message`}
                      </p>
                      <p className="mt-1 line-clamp-2 text-xs leading-5 text-[#8290A3]">
                        {readableMessage(message.body).slice(0, 180)}
                      </p>
                      <p className="mt-2 text-[11px] font-semibold text-[#9AA6B5]">
              ...
              <>
                <div className="flex flex-col gap-4 border-b border-[#E6EBF2] pb-5 sm:flex-row sm:items-start sm:justify-between">
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      <span className="text-xs font-black uppercase tracking-[.12em] text-[#55788B]">
                        {selected.channel}
                      </span>
                      {selectedCategory === "sale_intent" ? (
                        <span className="rounded-full bg-emerald-100 px-2.5 py-1 text-xs font-bold text-emerald-800">
                          Possible sale / payment step
                        </span>
                    ...
                    <h2 className="mt-3 font-display text-3xl font-bold tracking-[-.045em]">
                      {selected.subject || "Customer message"}
                    </h2>
                    <p className="mt-2 text-sm text-[#66758A]">
                      From{" "}
                      <strong className="text-[#33445B]">
                        {selected.senderReference}
                      </strong>{" "}
                  ...
                  <div className="flex flex-wrap gap-2">
                    {selected.contact?.id ? (
                      <Button
                        ...
                        onClick={() =>
                          navigate(
                            `/customers?contactId=${selected.contact!.id}`
                          )
                        }
                      >
                ...
                <div className="mt-6 rounded-2xl border border-[#E2E8F0] bg-[#FAFCFF] p-5 sm:p-6">
                  <p className="whitespace-pre-wrap text-sm leading-7 text-[#33445B]">
                    {selectedText || "This message has no readable body."}
                  </p>
                </div>

                <div className="mt-5 flex items-start gap-3 rounded-2xl bg-[#F3F7FF] p-4 text-sm leading-6 text-[#526985]">
                  <AlertCircle className="mt-0.5 h-5 w-5 shrink-0 text-[#55788B]" />
                  <p>
                    Reading this message does not mark anything read in the CRM
                    and does not send a response. Draft reply prepares work for
                    Review only.
                  </p>
                </div>
              </>
