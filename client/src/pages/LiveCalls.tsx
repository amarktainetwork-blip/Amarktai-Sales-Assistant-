...
export default function LiveCalls() {
  ...
  async function uploadChunk(blob: Blob, activeSessionId: number) {
    ...
    const response = await fetch("/api/live-calls/transcribe", {
      ...
      body: JSON.stringify({ audioBase64: base64, mimeType, durationMs: 5000 }),
    });
    ...
    if (result.signals?.length) {
      ...
      if (needsCoach && Date.now() - lastCoachAtRef.current > 15_000 && !coach.isPending) {
        ...
        // Persist every finalised transcript chunk even when no GenX coaching call is needed.
        // The existing coach endpoint owns transcript persistence, so use it only when semantic coaching is useful.
      }
    }
  }
    ...
    <div className="mt-6 grid gap-6 xl:grid-cols-[1.05fr_.95fr]">
      <section className="rounded-[1.5rem] border border-white/10 bg-[#0E2142] p-6">
        ...
        <div className="mt-6 min-h-48 rounded-xl border border-white/10 bg-[#08172F] p-4">
          <p className="text-[10px] font-black uppercase tracking-[.13em] text-[#7FAAF8]">LIVE TRANSCRIPT</p>
          <p className="mt-3 whitespace-pre-wrap text-sm leading-7 text-[#DCE6F6]">{transcript || "Finalised transcript chunks will appear here while the call is running."}</p>
        </div>
      </section>

      <div className="grid gap-6">
        <section className="rounded-[1.5rem] border border-white/10 bg-[#0E2142] p-6">
          <div className="flex items-center gap-3"><AlertTriangle className="size-5 text-[#83AEFF]"/><h2 className="font-display text-2xl font-bold tracking-[-.05em] text-white">Live signals</h2></div>
          <div className="mt-4 space-y-3">{signals.length ? signals.map((signal, index) => <article key={`${signal.type}-${index}`} className={`rounded-xl border p-4 ${signal.priority === "important" ? "border-[#4E8BFF]/50 bg-[#153B7A]" : "border-white/10 bg-[#0B1B37]"}`}><p className="text-xs font-black uppercase tracking-[.1em] text-[#8FB7FF]">{signal.label}</p><p className="mt-2 text-sm leading-6 text-[#D9E5F7]">{signal.evidence}</p></article>) : <p className="text-sm leading-6 text-[#9EB6DB]">Questions, objections, commitments, callback requests and buying signals detected by deterministic rules will appear here.</p>}</div>
        </section>

        <section className="rounded-[1.5rem] border border-white/10 bg-[#0E2142] p-6">
          <div className="flex items-center gap-3"><CheckCircle2 className="size-5 text-[#83AEFF]"/><h2 className="font-display text-2xl font-bold tracking-[-.05em] text-white">Current coaching</h2></div>
          <p className="mt-4 whitespace-pre-wrap text-sm leading-7 text-[#D9E5F7]">{tip || "A short coaching response appears only when an important signal or question warrants semantic help. Routine transcription does not call GenX."}</p>
        </section>
      </div>
    </div>
  </DashboardLayout>;
