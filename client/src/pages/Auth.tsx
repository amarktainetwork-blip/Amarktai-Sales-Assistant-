  ...
  const title = reset
    ...
    : isRegister
      ...
      : authView === "forgot"
        ...
        : "Welcome back.";
  const eyebrow = isRegister ? "CREATE ACCOUNT" : authView === "forgot" || reset ? "ACCOUNT RECOVERY" : "SECURE SIGN IN";
  ...
  return (
    <>
      ...
      <form onSubmit={submit} className="amk-auth-form">
        <Field name="invite-password" label="Password" value={password} onChange={setPassword} type="password" autoComplete="new-password" />
        <Field name="invite-confirm-password" label="Confirm password" value={confirm} onChange={setConfirm} type="password" autoComplete="new-password" />
...
function ManagedSignIn() {
  return (
    <>
      <p className="amk-auth__muted">Continue through your organisation's secure identity flow to open Amarktai Sales Assistant.</p>
      <button onClick={() => startLogin()} className="amk-auth__primary amk-auth__primary--spaced">Open Amarktai Sales Assistant <ArrowRight size={17}/></button>
      <Fineprint />
    </>
  );
}
...
function LocalRegistrationForm() {
  ...
  return (
    <form onSubmit={event => { event.preventDefault(); register.mutate({ name, email, password }); }} className="amk-auth-form">
      ...
      <Field name="register-name" label="Name" value={name} onChange={setName} autoComplete="name" />
      <Field name="register-email" label="Email" value={email} onChange={setEmail} type="email" autoComplete="email" />
      <Field name="register-password" label="Password" value={password} onChange={setPassword} type="password" autoComplete="new-password" />
      <button disabled={register.isPending} className="amk-auth__primary">
        {register.isPending ? "Creating…" : "Create account"} <ArrowRight size={17}/>
      </button>
...
function PasswordRecoveryForm() {
  ...
  return (
    <form onSubmit={event => { event.preventDefault(); request.mutate({ email }); }} className="amk-auth-form">
      ...
      <Field name="recovery-email" label="Email" value={email} onChange={setEmail} type="email" autoComplete="email" />
...
function ResetPasswordForm({ token }: { token: string }) {
  ...
  const reset = trpc.auth.resetPassword.useMutation({
    onSuccess: () => { toast.success("Password reset. Complete access verification to continue."); navigate("/dashboard"); },
    onError: error => toast.error(error.message),
  });
  ...
  return (
    <form onSubmit={event => { event.preventDefault(); if (password !== confirm) return toast.error("The passwords do not match."); reset.mutate({ token, password }); }} className="amk-auth-form">
      ...
      <Field name="reset-password" label="New password" value={password} onChange={setPassword} type="password" autoComplete="new-password" />
      <Field name="reset-confirm-password" label="Confirm password" value={confirm} onChange={setConfirm} type="password" autoComplete="new-password" />
...
function LocalLoginForm({ initialEmail = "" }: { initialEmail?: string }) {
  ...
  const login = trpc.auth.localLogin.useMutation({
    onSuccess: () => { toast.success("Signed in. Complete access verification to continue."); navigate("/dashboard"); },
    onError: error => toast.error(error.message),
  });

  return (
    <form onSubmit={event => { event.preventDefault(); login.mutate({ email, password }); }} className="amk-auth-form">
      <Field name="login-email" label="Email" value={email} onChange={setEmail} type="email" autoComplete="email" />
      <Field name="login-password" label="Password" value={password} onChange={setPassword} type="password" autoComplete="current-password" />
      <button type="submit" disabled={login.isPending} className="amk-auth__primary">
        {login.isPending ? "Signing in…" : "Open Amarktai Sales Assistant"} <ArrowRight size={17}/>
      </button>
      <div className="amk-auth__links">
        ...
        <button type="button" onClick={() => navigate("/auth?mode=register")} className="amk-auth__link">Create account</button>
      </div>
      <Fineprint />
    </form>
  );
...
function Field({ name, label, value, onChange, type = "text", autoComplete }: {
  name: string;
  label: string;
  value: string;
